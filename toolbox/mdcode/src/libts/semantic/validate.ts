...
function sqlExecutorErrors(action: Action, where: string): string[] {
  ...
  action.executor.sql.statements.forEach((stmt, i) => {
    ...
    if (text.slice(0, -1).includes(';')) {
      errors.push(`${at} contains ';'. Each statement runs on its own, so write ` +
          `one statement per list entry.`);
    }
    for (const name of referencedParameters(text)) {
      if (!bindable.has(name)) {
        errors.push(`${at} binds '@${name}', but action '${action.name}' ` +
...
// A hint for the common near-miss: the statement keys a row the action does
// create, but the action never declared that it creates it, so the runtime
