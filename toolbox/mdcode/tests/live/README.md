...
| `knowledge_catalog_live.test.ts` | Constraints survive a push and pull through a real Dataplex: the custom `semantic-constraint` type provisions, a server accepts one entry per constraint against it, and a pull recovers them byte for byte. |
