...
// A guard says the constraint decides whether the call may proceed. A
// constraint declaring `on_violation: warn` says the write proceeds anyway, so
// the pair reads as a gate while declaring that nothing is gated.
describe('a guard that does not gate is reported at load', () => {
  const reads = (onViolation?: string) => [{
    name: 'PositiveQuantity',
    expression: 'quantity > 0',
    ...(onViolation ? {on_violation: onViolation} : {}),
  }];
  const gateWarnings = (onViolation?: string) =>
      withGuards(['PositiveQuantity'], reads(onViolation))
          .warnings.filter(w => w.includes('gates nothing'));

  test('warns when the guarded constraint only warns', () => {
    const [w, ...rest] = gateWarnings('warn');
    expect(w).toBeDefined();
    expect(rest).toEqual([]);
    expect(w).toContain(`action 'PlaceOrder'`);
    expect(w).toContain(`'PositiveQuantity'`);
    expect(w).toContain(`on_violation 'warn'`);
  });

  test('stays silent for a constraint that does gate', () => {
    // `reject` and `escalate` both stop the write where it is -- one refuses
    // it, the other holds it -- so both are gates and neither is a slip.
    expect(gateWarnings('reject')).toEqual([]);
    expect(gateWarnings('escalate')).toEqual([]);
  });

  test('stays silent when the constraint states nothing', () => {
    // An unstated `on_violation` is read as `reject`, so an author who did not
    // say has not written this slip.
    expect(gateWarnings()).toEqual([]);
  });

  test('a warning constraint no action guards is not reported here', () => {
    // Nothing claims it gates, so there is no contradiction to report. It is
    // still a parameter-reading constraint nobody guards, which is the other
    // warning's business.
    const {warnings} = withGuards(undefined, reads('warn'));
    expect(warnings.some(w => w.includes('gates nothing'))).toBe(false);
  });
});
