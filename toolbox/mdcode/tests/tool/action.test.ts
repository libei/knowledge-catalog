...
// actionList()/actionRun()) -- the commands in front of the semantic runtime.
//
// Almost nothing here reaches a store, and that is not a compromise:
// `action-list` never opens one, and every run covered but the last fails
// before the first request. The exception fakes the Spanner client's own
// surface, because what it checks is the QUESTION the runtime asks the store.
// The argument parse, the choice of database, and the runtime's own refusal to
// run an action a constraint is supposed to decide all happen before a session
...
describe('kcmd action-list', () => {
  ...
  test('the run line names --judge when a guard is judged', async () => {
    ...
    //
    // `--skip-guards` is never offered here, however unrunnable the action
    // is without a judge. A listing that printed it would be recommending
    // the write with nothing checked, and the reader who pastes the line
    // would never learn the guard existed.
    ...
    const out = logs.join('\n');
    expect(out).toContain(
        'run:        kcmd action-run IssueCredit --judge --arg order=<Order>');
...
describe('kcmd action-run: what it will not send to a store', () => {
  ...
  test('refuses a guarded action while nothing evaluates the guard', async () => {
    writeWorkspace();
    const code =
...
describe('kcmd action-run: an action this binding cannot perform', () => {
  test('refuses an action whose executor the profile withdrew', async () => {
    ...
    writeWorkspace();
    const code = await actionRun(
        'IssueCredit', {profile: 'readonly', arg: ['order=1', 'amount=5']});
    expect(code).toBe(1);
    const out = logs.join('\n');
...
describe('kcmd action: what the command line can actually contain', () => {
  ...
  test('a named profile still selects that profile', async () => {
    writeWorkspace();
    await actionList(undefined, {profile: 'analytical'});
    expect(logs.join('\n')).toContain('profile \'analytical\'');
  });
  ...
  test('a genuinely repeated argument is still reported', async () => {
    writeWorkspace();
    const code = await actionRun(
        'IssueCredit', {arg: ['order=1', 'order=2', 'amount=5']});
    expect(code).toBe(1);
...
describe('kcmd action-run: a subtype inherits its fields', () => {
  test('resolves a reference by an inherited identifying column', async () => {
    ...
    expect(await actionRun('Touch', {arg: 'who=Alice'})).toBe(1);
    expect(asked[0]).toContain('FullName = @ref');
  });
});


describe('kcmd action-run: --skip-guards', () => {
  test('runs a guarded action that would otherwise be refused', async () => {
    // The point of the flag. An author trying their own model against their
    // own database has no judge to stand up, and every refusal a guard
    // produces is total -- so without this they would be deleting the guard
    // to test the write, which loses the guard and tests a different model.
    writeWorkspace();
    const asked: string[] = [];
    const ok = (result: unknown) =>
        Promise.resolve({status: 200, result} as any);
    ...
    await actionRun(
        'IssueCredit', {arg: ['order=12345', 'amount=30'], skipGuards: true});
    const out = logs.join('\n');
    // No longer stopped by the guard, and it reached the store: the reference
    // lookup is a question only a run that got past the guard would ask.
    expect(out).not.toContain('is guarded by \'CreditIsPositive\'');
    expect(asked.length).toBeGreaterThan(0);
    // And it says what it did not do, on its own line, so the reader is never
    // left to remember which flag they typed.
    expect(out).toContain('no guard is checked and the write still happens');
  });

  test('is refused together with --judge', async () => {
    // Two answers to one question. Enforcing either silently would enforce
    // less, or more, than the caller asked for.
    writeWorkspace();
    const code = await actionRun(
        'IssueCredit',
        {arg: ['order=12345', 'amount=30'], skipGuards: true, judge: true});
    expect(code).toBe(1);
    expect(logs.join('\n')).toContain('opposite instructions');
  });
});
